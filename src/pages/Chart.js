...
const CustomizedActiveDot = (props) => {
  ...
  if (props.data[index].note) {
    const color = props.data[index].color
    return (
      <circle r={r} stroke="#000" stroke-strokeWidth="1" fill={color} width={width} height={height} className="recharts-dot recharts-line-dot" cx={cx} cy={cy}/>
    );
  } else {
    const color = "#78909c"
    return (
      <circle r={r} stroke="#000" strokeWidth="1" fill={color} width={width} height={height} className="recharts-dot recharts-line-dot" cx={cx} cy={cy}/>
    );
  }
}
    ...
    <React.Fragment>
      <ResponsiveContainer>
        <LineChart data={data} margin={{top: 24, right: 16, bottom: 0, left: -24}}>
          ...
          <XAxis dataKey="date" tickLine={false} tick={<CustomizedAxisTick/>} ticks={['2019/11/04', '2019/11/05', '2019/11/06', '2019/11/07']}/>
